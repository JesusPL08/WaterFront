...
@Component({
  ...
  templateUrl: './create-bonus.page.html',
  styleUrls: ['./create-bonus.page.scss']
})
export class CreateBonusPage implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<CreateBonusPage>,
  ...
  registrarBonos(): void {
    ...
    for (const bonus of bonusesToSend) {
      this.bonusService.create(bonus).subscribe({
        next: () => {
          enviados++;
          if (enviados === bonusesToSend.length && !huboError) {
            this.snackBar.open('✔️ Bonos registrados con éxito', 'Cerrar', {
              duration: 3000,
        ...
        error: (err) => {
          huboError = true;
          const msg = err?.error?.message || 'Error al registrar bonos';
